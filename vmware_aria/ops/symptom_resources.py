...
def _walk_symptom_instances(client: AriaClient, wanted: set[str]) -> tuple[dict[str, dict], bool]:
    """Instance rows for ``wanted`` symptom ids, and whether every page was read.

    Raises:
        TypeError: A page answered without a ``symptom`` list.
    """
    found: dict[str, dict] = {}
    seen: set[str] = set()
    page = 0
    while True:
        ...
        if not isinstance(rows, list):
            raise TypeError("GET /symptoms answered without a 'symptom' list")
        new_rows = 0
        for row in rows:
        ...
        if len(rows) < _SYMPTOM_PAGE_SIZE or (isinstance(total, int) and len(seen) >= total):
            return found, True
        if not new_rows or len(seen) >= _SYMPTOM_MAX_TOTAL:
            # A page of nothing new means the page parameter was ignored too.
...
def _locate(client: AriaClient, symptoms: list[dict]) -> list[dict]:
    ...
    if wanted:
        try:
            instances, complete = _walk_symptom_instances(client, wanted)
        except Exception as exc:  # noqa: BLE001 — reported per symptom, not swallowed
            _log.warning("Could not read symptom instances: %s", exc)
            complete = False
    located = []
    for s in symptoms:
        if s["resource_id"]:
