...
class TargetConfig:
    ...
    def get_username(self, target_name: str) -> str:
        ...
        Convention: VMWARE_ARIA_<TARGET>_USERNAME
        where <TARGET> is upper-cased with hyphens replaced by underscores.
        ...
        ``auth_source`` stays in config.yaml: it selects the authentication
        realm rather than identifying the account, so it does not rotate.
        """
        env_key = f"VMWARE_ARIA_{target_name.upper().replace('-', '_')}_USERNAME"
...
def resolve_config_path(config_path: Path | None = None) -> Path:
    ...
    again in :func:`vmware_aria.doctor.run_doctor`, and the doctor's copy had no
    env-var clause — so with ``VMWARE_ARIA_CONFIG`` set it reported on a file
    the tools would never open, and passed (2026-08-30). Two copies of a rule
    do not disagree loudly; they disagree slowly (形态 #6).
    """
