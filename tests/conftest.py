...
* ``HOME`` moves the per-skill JSON Lines log under ``~/.vmware-aria``, which
  resolves through ``Path.home()`` and ignores ``OPS_HOME`` entirely.

Before this, running the suite appended 5 rows per run to the operator's
...
SANDBOX_HOME = Path(tempfile.mkdtemp(prefix="vmware-aria-tests-"))
