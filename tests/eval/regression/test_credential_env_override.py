...
# Target "prod" -> VMWARE_ARIA_PROD_{USERNAME,PASSWORD}
_USER_KEY = "VMWARE_ARIA_PROD_USERNAME"
_PW_KEY = "VMWARE_ARIA_PROD_PASSWORD"


def _target() -> TargetConfig:
    return TargetConfig(host="aria.example.com", username="config-admin")
...
def test_hyphenated_target_maps_to_underscored_key(monkeypatch):
    ...
    targets' variables."""
    monkeypatch.setenv("VMWARE_ARIA_ARIA_LAB_USERNAME", "lab-admin")
    assert _target().get_username("aria-lab") == "lab-admin"


def test_auth_source_still_comes_from_config(monkeypatch):
    """auth_source selects the realm, not the account — it is deliberately not
    part of the rotating pair and must keep reading from config."""
    monkeypatch.setenv(_USER_KEY, "vault-admin")
    target = TargetConfig(host="h", username="config-admin", auth_source="LDAP")
    assert target.get_username("prod") == "vault-admin"
    assert target.auth_source == "LDAP"
...
def test_connection_manager_authenticates_with_env_username(monkeypatch):
    ...
    monkeypatch.setattr("vmware_aria.connection.AriaClient", _fake_client)
    monkeypatch.setenv(_USER_KEY, "vault-admin")
