...
def test_a_client_can_be_built_against_a_self_signed_target(no_urllib3, monkeypatch):
    ...
    monkeypatch.setattr(conn.AriaClient, "_acquire_token", lambda self: None)

    client = conn.AriaClient(_target(verify_ssl=False), "secret")
...
def test_a_verifying_target_is_unaffected(no_urllib3, monkeypatch):
    ...
    client = conn.AriaClient(_target(verify_ssl=True), "secret")
