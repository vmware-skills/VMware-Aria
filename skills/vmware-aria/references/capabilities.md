...
| `GET /suite-api/api/symptoms` | get_alert / investigate_alert (the object each symptom is on — `resourceId`, `statKey`, `message` — when the contributing-symptom leaf carries none; 8.18.7 ignores the `id` filter, so pages are walked until every symptom is found) |
