...
**Example**: `LOCATOR ERROR`, everything else OK → DEGRADED. `health node` showed 7.61 of 7.75 GB used and swap in use, so the VM was resized to 16 GB. Afterwards `health node` read 15.61 GB total, 41% actually free, pressure NORMAL, all watchdog restarts 0 — and LOCATOR was **still** in ERROR. Memory pressure was real but not the cause. LOCATOR is the GemFire locator on port 6061; Broadcom KB 404805 describes it failing when a node's IP changed and the analytics configuration still points at the old address (check `/storage/vcops/log/analytics-*.log` for "Could not contact any of the locators").
