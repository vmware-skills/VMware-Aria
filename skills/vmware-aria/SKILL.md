...
```bash
uv tool install vmware-aria==1.14.1
...
`uvx` re-resolves dependencies from PyPI on every launch. Behind a corporate TLS-intercepting proxy whose CA is not in uv's bundled cert store, the handshake fails. Use the v1.5.15+ recommended single-command form `vmware-aria mcp` (after `uv tool install vmware-aria==1.14.1` — no network on launch), or set `UV_NATIVE_TLS=true` to make uv use the system cert store.
